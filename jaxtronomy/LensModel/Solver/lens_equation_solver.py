...
class LensEquationSolver(object):
    ...
    def image_position_stochastic(
        ...
        for i in range(num_random):
            x_init = (
                jnp.random.uniform(-search_window / 2.0, search_window / 2) + x_center
            )
            y_init = (
                jnp.random.uniform(-search_window / 2.0, search_window / 2) + y_center
            )
            xinitial = jnp.array([x_init, y_init])
            result = minimize(
                self._root,
                xinitial,
    ...
    def image_position_analytical(
        ...
        **kwargs_solver,
        ...
        if magnification_limit is not None:
            mag = jnp.abs(self.lensModel.magnification(x_mins, y_mins, kwargs_lens))
    ...
    def image_position_lenstronomy(
        ...
        if initial_guess_cut:
            ...
            print(mag)
            print(type(mag))
            # mag[mag < 1] = 1 # NH: can't do this in jax
    ...
    def _find_gradient_decent(
        ...
        :param x_min: jnp.array, list of 'good guess' solutions of the lens equation
        :param y_min: jnp.array, list of 'good guess' solutions of the lens equation
    ...
    def _solve_single_proposal(
        ...
        l = 0
        if non_linear:
            xinitial = jnp.array([x_guess, y_guess])
            result = minimize(
                self._root,
                xinitial,
                args=(kwargs_lens, source_x, source_y),
    ...
    def _gradient_step(
        ...
        if delta_new > delta_init:
            if num_iter_max < iter_num:
                ...
                # image_plane_vector[0] *= jnp.random.normal(loc=0, scale=0.5) # NH: can't do this in jax
                # image_plane_vector[1] *= jnp.random.normal(loc=0, scale=0.5)
                # x = x.at[idx].set(y)
                # x = x.at[idx].multiply(y)
                image_plane_vector[0] = image_plane_vector.at[0].multiply(jnp.random.normal(loc=0, scale=0.5))
                image_plane_vector[1] = image_plane_vector.at[1].multiply(jnp.random.normal(loc=0, scale=0.5))
    ...
    def sort_arrival_times(self, x_mins, y_mins, kwargs_lens):
        ...
        idx = jnp.argsort(arrival_time)
        x_mins = jnp.array(x_mins)[idx]
        y_mins = jnp.array(y_mins)[idx]
        return x_mins, y_mins
